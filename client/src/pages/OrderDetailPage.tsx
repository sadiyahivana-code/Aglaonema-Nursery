...
import { FileDown, ArrowLeft, X } from "lucide-react";
...
const pdfStyles = StyleSheet.create({
  ...
  tableHeader: { flexDirection: "row", backgroundColor: "#1B4332", padding: 8 },
  tableHeaderText: {
    color: "#F8F4EE",
    fontSize: 8,
    fontFamily: "Helvetica-Bold",
      ...
      <Page size="A4" style={pdfStyles.page}>
        ...
        <View style={pdfStyles.header}>
          ...
          <View>
            <Text style={pdfStyles.storeInfo}>
              Jl. Desa No.51, Waru, Parung, Bogor 16330
            </Text>
            <Text style={pdfStyles.storeInfo}>
              (0251) 8123-456 | hello@aglaonema.id
            </Text>
        ...
        <View style={pdfStyles.infoBox}>
          ...
          <View style={pdfStyles.infoCard}>
            ...
            <Text
              style={[
                pdfStyles.infoValue,
                { color: isPaid ? "#1B4332" : "#D97706" },
              ]}
            >
              {isPaid ? "LUNAS" : "MENUNGGU"}
            </Text>
            <Text style={[pdfStyles.infoLabel, { marginTop: 6 }]}>
              Metode Bayar
            </Text>
            <Text style={pdfStyles.infoValue}>{order.paymentMethod}</Text>
          </View>
          <View style={pdfStyles.infoCardLast}>
            ...
            <Text
              style={[pdfStyles.infoValue, { fontSize: 7, lineHeight: 1.4 }]}
            >
        ...
        <View style={pdfStyles.table}>
          <View style={pdfStyles.tableHeader}>
            ...
            <Text style={[pdfStyles.tableHeaderText, pdfStyles.col2]}>
              Nama Produk
            </Text>
            <Text style={[pdfStyles.tableHeaderText, pdfStyles.col3]}>Qty</Text>
            <Text style={[pdfStyles.tableHeaderText, pdfStyles.col4]}>
              Harga Satuan
            </Text>
            <Text style={[pdfStyles.tableHeaderText, pdfStyles.col5]}>
              Subtotal
            </Text>
          </View>
          {order.items.map((item: any, i: number) => (
            ...
            >
              ...
              <Text style={[pdfStyles.tableText, pdfStyles.col2]}>
                {item.product.name}
              </Text>
              <Text style={[pdfStyles.tableText, pdfStyles.col3]}>
                {item.quantity}
              </Text>
              <Text style={[pdfStyles.tableText, pdfStyles.col4]}>
                {formatRupiah(item.price)}
              </Text>
              <Text style={[pdfStyles.tableText, pdfStyles.col5]}>
        ...
        <View style={pdfStyles.totalsContainer}>
          <View style={pdfStyles.totalRow}>
            ...
            <Text style={pdfStyles.totalValue}>
              {formatRupiah(order.totalAmount - order.shippingCost)}
            </Text>
          </View>
          <View style={pdfStyles.totalRow}>
        ...
        <Text style={pdfStyles.footer}>
          ...
          Untuk pertanyaan, hubungi kami di (+62) 85797159921 atau
          hello@aglaonema.id{"\n"}
...
function CancelModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative bg-white rounded-sm border border-cream-300 p-6 max-w-sm w-full shadow-xl">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-cream-500 hover:text-bark-900"
        >
          <X className="w-4 h-4" />
        </button>
        ...
        <p className="font-body text-sm text-cream-700 mb-6">
          Pesanan yang sudah dibatalkan tidak bisa dikembalikan. Yakin ingin
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={onConfirm}
            disabled={isLoading}
            className="flex-1 bg-red-600 hover:bg-red-700 text-white text-sm font-body font-semibold py-2 px-4 rounded-sm transition-colors disabled:opacity-50"
          >
...
export default function OrderDetailPage() {
  ...
  if (!order)
    return (
      <div className="text-center py-20 font-display text-xl">
  ...
  const detectedKurir = order.shippingMethod?.toLowerCase().includes("sicepat")
    ? "sicepat"
    : order.shippingMethod?.toLowerCase().includes("jnt") ||
        order.shippingMethod?.toLowerCase().includes("j&t")
      ? "jnt"
      : "jne";
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
      ...
      <button
        ...
        className="flex items-center gap-1 text-sm font-body text-cream-700 hover:text-forest-800 mb-6 transition-colors"
      >
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
        <div>
          <h1 className="font-display text-2xl text-forest-800">
            {order.invoiceNumber}
          </h1>
          <p className="font-body text-sm text-cream-600 mt-1">
            {formatDate(order.createdAt)}
          </p>
        </div>
        <div className="flex items-center gap-3">
          <span
            className={`px-3 py-1.5 rounded-sm text-xs font-body font-semibold ${st.color}`}
          >
            {st.label}
          </span>
          {order.status === "PENDING" && (
            <button
              ...
              className="px-3 py-1.5 rounded-sm text-xs font-body font-semibold bg-red-50 text-red-600 hover:bg-red-100 transition-colors"
            >
              Batalkan
            </button>
          )}
      ...
      {order.status !== "CANCELLED" && (
        <div className="bg-white border border-cream-300 rounded-sm p-6 mb-6">
          <h2 className="font-display text-base text-forest-800 mb-5">
            Status Pengiriman
          </h2>
          <div className="flex items-center">
            {timeline.map((step, i) => {
              ...
              return (
                ...
                >
                  <div
                    className={`flex flex-col items-center ${i < timeline.length - 1 ? "flex-1" : ""}`}
                  >
                    <div
                  ...
                  {i < timeline.length - 1 && (
                    <div
                      className={`flex-1 h-0.5 mx-2 -mt-5 ${done ? "bg-forest-700" : "bg-cream-300"}`}
                    />
                  )}
                </div>
              );
          ...
          {/* Tracking Widget — muncul saat status SHIPPED dan ada resi */}
          {order.trackingNumber && order.status === "SHIPPED" && (
            <div className="mt-4 border-t border-cream-300 pt-4">
              <TrackingWidget
                defaultResi={order.trackingNumber}
                defaultKurir={detectedKurir}
              />
            </div>
          )}
        </div>
      ...
      {/* Items */}
      <div
        className="bg-white border border-cream-300 rounded-sm p-6 mb-6"
        id="invoice"
      >
        <h2 className="font-display text-base text-forest-800 mb-4">
          Produk yang Dipesan
        </h2>
          ...
          {order.items.map((item: any) => (
            <div key={item.id} className="flex gap-3">
              ...
              <div className="flex-1">
                <div className="font-body font-medium text-sm text-bark-900">
                  {item.product.name}
                </div>
              ...
              <div className="font-body font-bold text-sm text-forest-800">
                {formatRupiah(item.price * item.quantity)}
              </div>
            </div>
          ))}
      ...
      <div className="bg-white border border-cream-300 rounded-sm p-5">
        <h2 className="font-display text-base text-forest-800 mb-2">
          Alamat Pengiriman
        </h2>
        <p className="font-body text-sm text-bark-800 whitespace-pre-line">
          {order.shippingAddress}
        </p>
        {order.notes && (
          <p className="font-body text-sm text-cream-600 mt-2 italic">
            Catatan: {order.notes}
          </p>
        )}
      </div>
