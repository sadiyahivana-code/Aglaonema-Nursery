...
export default function StorePage() {
  ...
  const defaultCenter: [number, number] = stores[0]
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-8">
        <p className="text-gold-500 tracking-[0.3em] text-xs font-body uppercase mb-1">
          Kunjungi Kami
        </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-4">
          {stores.map((store: any) => (
            <div
              key={store.id}
              className="bg-white border border-cream-300 rounded-sm p-5"
            >
              <h3 className="font-display font-semibold text-forest-800 text-base mb-3">
                {store.name}
              </h3>
              <div className="space-y-2 font-body text-sm">
                <div className="flex gap-2">
                  <MapPin className="w-4 h-4 text-gold-500 flex-shrink-0 mt-0.5" />
                  <span className="text-bark-800 leading-relaxed">
                    {store.address}
                  </span>
                </div>
                <div className="flex gap-2">
          ...
          <button
            onClick={() => {
              navigator.geolocation.getCurrentPosition((pos) => {
                window.open(
                  `https://www.google.com/maps/dir/${pos.coords.latitude},${pos.coords.longitude}/${defaultCenter[0]},${defaultCenter[1]}`,
                  "_blank",
                );
              });
            }}
              ...
              >
                <Popup>
                  <div className="font-body p-1 min-w-[200px]">
                    <div className="font-semibold text-forest-800 mb-1">
                      {store.name}
                    </div>
